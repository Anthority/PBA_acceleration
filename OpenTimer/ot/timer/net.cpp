...
namespace ot
{
  ...
  // Constructor
  RctEdge::RctEdge(RctNode &from, RctNode &to, float res) : _from{from},
                                                            _to{to},
                                                            _res{res}
  {
  }
  ...
  void Net::_make_rct()
  {
    ...
    // 如果这个成员为空（即没有SPEF网络），那么方法就直接返回，不做任何操作
    if (!_spef_net)
      return;
  ...
  void Net::_scale_capacitance(float s)
  {
    ...
    std::visit(Functors{// Leave this to the next update timing
                        [&](EmptyRct &rct) {
                        },
                        [&](Rct &rct)
                        {
  ...
  void Net::_update_rc_timing()
  {
    ...
    std::visit(Functors{[&](EmptyRct &rct)
                        {
                          // 如果_rct是一个EmptyRct，那么代码会计算每个引脚的电容，并将结果累加到rct.load[el][rf]中。
                          FOR_EACH_EL_RF(el, rf)
                          {
  ...
  float Net::_load(Split m, Tran t) const
  {
    ...
    return std::visit(Functors{[&](const EmptyRct &rct)
                               {
                                 return rct.load[m][t];
                               },
                               [&](const Rct &rct)
  ...
  std::optional<float> Net::_slew(Split m, Tran t, float si, Pin &to) const
  {
    ...
    return std::visit(Functors{[&](const EmptyRct &) -> std::optional<float>
                               {
                                 return si;
                               },
