...
namespace ot
{
  ...
  // Constructor
  Pin::At::At(Arc *a, Split el, Tran rf, float v) : pi_arc{a},
                                                    pi_el{el},
                                                    pi_rf{rf},
                                                    numeric{v}
  {
  ...
  // Constructor
  Pin::Slew::Slew(Arc *a, Split el, Tran rf, float v) : pi_arc{a},
                                                        pi_el{el},
                                                        pi_rf{rf},
                                                        numeric{v}
  {
  ...
  // Constructor
  Pin::Rat::Rat(Arc *a, Split el, Tran rf, float v) : pi_arc{a},
                                                      pi_el{el},
                                                      pi_rf{rf},
                                                      numeric{v}
  {
  ...
  bool Pin::has_self_loop() const
  {
    ...
    return std::find_if(arcs.begin(), arcs.end(), [](auto arc)
                        { return arc->is_self_loop(); }) != arcs.end();
  }

  // Function: is_input
  bool Pin::is_input() const
  {
    return std::visit(Functors{[](PrimaryInput *)
                               {
                                 return true;
                               },
                               [](PrimaryOutput *)
