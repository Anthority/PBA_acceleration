...
#ifdef _CAL_ARC_TIMING
  std::cout << "FULL PBA 重新计算ARC总用时:" << float(pba_timing_elapsed_time) / 1000000 << " milliseconds"
			<< std::endl;
  std::cout << "FULL PBA 重新计算单条ARC用时:" << float(pba_timing_elapsed_time) / recal_num << " nanoseconds"
			<< std::endl;
#endif
}
...
void Timer::report_timing_pba_merge(std::vector<Path> &paths, float acceptable_slew, size_t min_length) {
  ...
  for (auto &path : paths) {
	...
	size_t last_merge = path.size() - 1;
	size_t now_merge = path.size() - 1;
...
#ifdef _CAL_ARC_TIMING
  std::cout << "MERGE PBA 重新计算ARC总用时:" << float(pba_timing_elapsed_time) / 1000000 << " milliseconds"
			<< std::endl;
  std::cout << "MERGE PBA 重新计算单条ARC用时:" << float(pba_timing_elapsed_time) / recal_num << " nanoseconds"
			<< std::endl
			<< std::endl;
#endif
  ...
  std::cout << "MERGE PBA 复制路径单条ARC用时:" << float(copy_timing_elapsed_time) / copy_seg_num << " nanoseconds"
			<< std::endl
			<< std::endl;
