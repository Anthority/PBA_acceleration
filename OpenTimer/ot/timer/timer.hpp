...
namespace ot
{
  ...
  private:
    ...
    // void _topologize_reverse(SfxtCache &, size_t) const;
    ...
    void _my_recover_datapath(Path &, const SfxtCache &, const PfxtNode *, size_t) const;
    ...
    void _my_spur(Endpoint &, size_t, PathHeap &) const;
    void _my_spur(PfxtCache &, const PfxtNode &) const;
    ...
    SfxtCache _my_sfxt_cache(const Endpoint &) const;
    SfxtCache _my_sfxt_cache(const PrimaryOutput &, Split, Tran) const;
    SfxtCache _my_sfxt_cache(const Test &, Split, Tran) const;
    ...
    PfxtCache _my_pfxt_cache(const SfxtCache &) const;
  ...
  // 将pin按照从rise到fall的顺序编码
  inline auto Timer::_encode_pin(const Pin &pin, Tran rf) const
  {
